...
describe('getCostSummary', () => {
  it('returns all zeros on empty DB (for test agents)', async () => {
    const result = await getCostSummary(prisma);
  ...
  it('throws on invalid date range (fromDate > toDate)', async () => {
    await expect(
      getCostSummary(prisma, '2026-03-21', '2026-03-01'),
...
describe('getCostTimeline', () => {
  ...
  it('zero-fills days with no activity', async () => {
    ...
    const result = await getCostTimeline(prisma, 7);
    const agentSeries = result.series.find((s) => s.agentId === testAgentId);
    if (agentSeries) {
      expect(agentSeries.dailyCosts).toHaveLength(7);
      const zeroCount = agentSeries.dailyCosts.filter((c) => c === 0).length;
  ...
  it('returns single series when filtered by agentId', async () => {
    ...
    const result = await getCostTimeline(prisma, 7, testAgentId);
  ...
  it('returns empty series when no data exists', async () => {
    const result = await getCostTimeline(prisma, 7, '00000000-0000-0000-0000-000000000099');
...
describe('getUsageStats', () => {
  it('returns correct run, call, and approval counts', async () => {
    ...
    const result = await getUsageStats(prisma);
  ...
  it('returns all zeros on empty DB (no test data)', async () => {
    const result = await getUsageStats(prisma, '2099-01-01', '2099-01-02');
...
describe('getAgentLeaderboard', () => {
  it('sorts by cost descending', async () => {
    ...
    const result = await getAgentLeaderboard(prisma, 'cost', 10);
    expect(result.agents.length).toBeGreaterThanOrEqual(2);

    const idx1 = result.agents.findIndex((a) => a.agentId === testAgentId);
    const idx2 = result.agents.findIndex((a) => a.agentId === testAgent2Id);
    if (idx1 >= 0 && idx2 >= 0) {
  ...
  it('sorts by errorRate descending', async () => {
    ...
    const result = await getAgentLeaderboard(prisma, 'errorRate', 10);
    expect(result.agents.length).toBeGreaterThanOrEqual(2);

    const agent2Entry = result.agents.find((a) => a.agentId === testAgent2Id);
  ...
  it('includes healthScore for each agent', async () => {
    ...
    const result = await getAgentLeaderboard(prisma, 'cost', 10);
    const entry = result.agents.find((a) => a.agentId === testAgentId);
  ...
  it('respects limit parameter', async () => {
    ...
    const result = await getAgentLeaderboard(prisma, 'cost', 1);
  ...
  it('returns empty agents on no data', async () => {
    await prisma.auditLog.deleteMany({ where: { agentId: { in: [testAgentId, testAgent2Id] } } });
    const result = await getAgentLeaderboard(prisma, 'cost', 10);
    const hasTestAgents = result.agents.some((a) =>
...
describe('getModelUsage', () => {
  it('returns model breakdown sorted by cost desc', async () => {
    ...
    const result = await getModelUsage(prisma);
    expect(result.models.length).toBeGreaterThanOrEqual(2);

    const claudeIdx = result.models.findIndex((m) => m.model === 'claude-sonnet-4-5');
    const gptIdx = result.models.findIndex((m) => m.model === 'gpt-4o');
  ...
  it('excludes null model entries', async () => {
    ...
    const result = await getModelUsage(prisma);
    const nullModel = result.models.find((m) => m.model === null || m.model === '');
