...
describe('evaluatePolicy', () => {
  it('returns DENY when a DENY rule matches', async () => {
    ...
    const result = await evaluatePolicy(prisma, testAgentId, 'delete_record', 'HIGH');
  ...
  it('DENY wins over ALLOW when both match', async () => {
    ...
    const result = await evaluatePolicy(prisma, testAgentId, 'send_email', 'HIGH');
  ...
  it('REQUIRE_APPROVAL wins over ALLOW', async () => {
    ...
    const result = await evaluatePolicy(prisma, testAgentId, 'send_email', 'LOW');
  ...
  it('returns default REQUIRE_APPROVAL when no rules match', async () => {
    const result = await evaluatePolicy(prisma, testAgentId, 'unknown_action_xyz', 'MEDIUM');
  ...
  it('wildcard actionType "*" matches any action', async () => {
    ...
    const result = await evaluatePolicy(prisma, testAgentId, 'any_random_action', 'MEDIUM');
  ...
  it('empty riskTiers array matches all tiers', async () => {
    ...
    const result = await evaluatePolicy(prisma, testAgentId, 'shutdown', 'LOW');
    expect(result.effect).toBe('DENY');

    const result2 = await evaluatePolicy(prisma, testAgentId, 'shutdown', 'CRITICAL');
  ...
  it('inactive policy is skipped during evaluation', async () => {
    ...
    const result = await evaluatePolicy(prisma, testAgentId, 'any_action', 'HIGH');
  ...
  it('agent-specific policy is evaluated before global', async () => {
    ...
    const result = await evaluatePolicy(prisma, testAgentId, 'deploy', 'HIGH');
  ...
  it('throws error for non-existent agent', async () => {
    await expect(
      evaluatePolicy(prisma, '00000000-0000-0000-0000-000000000000', 'any', 'LOW'),
