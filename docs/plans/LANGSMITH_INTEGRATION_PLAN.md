...
| P1 (schema + SDK plumbing) | PR2 | **Done** (2026-05-01) | `chore: add LangSmith cross-link columns + indexes`. Adds `AuditLog.langsmithRunId` (indexed) + `langsmithProject`, and `Agent.langsmithEnabled` (default `false`) + `langsmithProject`. Migration `20260502000000_add_langsmith_crosslink` is additive-only (nullable cols / safe defaults). `AuditEventSchema` gains `LangSmithRunIdSchema` (`^[A-Za-z0-9_-]+$`, max 64) and `LangSmithProjectSchema` (`^[A-Za-z0-9_\\-/.]+$`, max 128) — strict regex blocks log-injection / XSS payloads. `AuditLogEntry` DTO + `PrismaAuditRepository` + `MockAuditRepository` + `AuditService` all surface the fields end-to-end. SDK's `LLMCallMetadata` accepts the two fields and `wrapLLMCall` / `wrapLLMStream` propagate them via `logEvent` (omitted entirely when caller doesn't supply them — no null leakage on the wire). 11 new API integration tests (round-trip via `/audit/log` + `/audit/batch`, 400s for XSS, newline injection, oversized, empty, slash-overflow, space-in-project) and 4 new SDK unit tests (success path, failure path, stream path, no-leak). Full sweep: API 313/313, SDK 47/47, web `tsc -b` clean. |
| P1 (web cross-link UI) | PR3 | **Done** (2026-05-01) | `feat(web): "View in LangSmith" link in TraceDrawer behind env flag`. New `apps/web/src/lib/langsmith.ts` builds the deep-link URL purely (`${base}[/o/<org>][/projects/p/<project>]/r/<runId>`) — base must parse as `http(s)://`, every path segment is `encodeURIComponent`d, and the builder returns `null` (renders no link) on invalid input, so a typo'd env var can never produce an unsafe href. `TraceDrawer.tsx` extends `TraceEvent` with `langsmithRunId`/`langsmithProject` and gates the badge on three checks: dashboard configured, event is `llm_call`, row has a server-validated `langsmithRunId`. Anchor uses `target="_blank" rel="noopener noreferrer"`. New `apps/web/src/vite-env.d.ts` types `VITE_LANGSMITH_UI_BASE` and `VITE_LANGSMITH_ORG_ID`; both documented in `.env.example`. The org segment is optional so self-hosted LangSmith (no org-scoped routing) works out of the box. Verified: `tsc -b` clean, ESLint clean on all touched files (32 pre-existing errors in untouched files unchanged), full API+SDK suites still green (313+47). |
| P2 | — | Not started | |
| P3 | PR4 | **Done** (2026-05-02) | `feat(sdk): optional LangSmith fanout in wrapLLMCall`. New `packages/governance-sdk/src/langsmith.ts` exports `createLangSmithBridge(config)` returning `{ project, mintRunId, recordLLM, shutdown, getMetrics }`. Bridge is opt-in via `GovernanceClientConfig.langsmith` and isolated from the AgentOS pipeline: dedicated `EventBuffer`, dedicated `CircuitBreaker`. Diverged from plan in three documented ways: (1) calls LangSmith REST directly (`POST {baseUrl}/runs/batch`) instead of lazy-loading the `langsmith` npm package — eliminates a peer-dep matrix and bundle bloat; (2) added optional `inputs?: unknown` to `LLMCallMetadata` since `wrapLLMCall(fn)` can't see prompts through the function alone; (3) sends "complete-run-in-one-shot" entries (`post[]` only, empty `patch[]`) instead of separate start+end calls — simpler retry, matches what the LangSmith JS SDK does internally. Security: redact callback runs before serialisation; `metadataOnly: true` strips inputs/outputs entirely; `maxPayloadBytes` (default 64 KB) replaces oversized payloads with a `__truncated` marker; circular refs replaced with `__unserialisable`; redactor-throws is caught and logged without echoing payload; **`scrubApiKey` defends against verbatim-in-error-message leaks** (caught by a regression test). `wrapLLMCall` mints runId before `fn()` so failures are cross-linked too; both success and failure paths post to LangSmith and stamp the same id onto the AgentOS audit event. `wrapLLMStream` mints runId at the top and posts the collected chunks once after stream completion (or `error` on mid-stream throw). `getMetrics().langsmith` returns `{enabled:false}` when unconfigured or `{enabled:true, pending, dropped, breaker, lastFlushMs}` when on. `shutdown()` drains both buffers via `Promise.allSettled` so a hung LangSmith can't block the audit drain. 27 new bridge unit tests (constructor validation, mint, wire format, redact, metadataOnly, payload cap, breaker, key-not-in-error-logs) + 8 new integration tests in `GovernanceClient.test.ts` (cross-link runId, failure path, stream fanout, no-op when unconfigured, 5xx outage doesn't break audit, metrics shape, shutdown isolation). Full sweep: SDK 89/89 in 4 files, API 313/313 in 22 files, types + web tsc clean. |
| P4 | — | Not started | |
| P5 | — | Not started | |
| P6 | — | Not started | |
