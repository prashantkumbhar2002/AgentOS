...
| P1 (web cross-link UI) | PR3 | Not started | `feat(web): "View in LangSmith" link in TraceDrawer behind env flag`. Reads `langsmithRunId` from each `llm_call` event, renders an external link to `${VITE_LANGSMITH_UI_BASE}/o/<org>/projects/p/<encoded-project>/r/<runId>` with `rel="noopener noreferrer"`. Hidden when `VITE_LANGSMITH_UI_BASE` is unset. |
| P2 | — | Not started | |
| P3 | — | Not started | |
| P4 | — | Not started | |
| P5 | — | Not started | |
